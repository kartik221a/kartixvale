#!/usr/bin/env node
/**
 * Seed script for the blogs table in Turso database.
 * 
 * Usage:
 *   TURSO_DATABASE_URL=libsql://your-db.turso.io TURSO_AUTH_TOKEN=your-token node scripts/seed-blogs.mjs
 *
 * This script:
 * 1. Creates the `blogs` table if it doesn't exist
 * 2. Reads blog data from blog-data.json
...
async function seedBlogs() {
  ...
  const dataPath = join(__dirname, 'blog-data.json');
...
async function main() {
  try {
    if (!process.env.TURSO_DATABASE_URL && !process.env.DATABASE_URL) {
      ...
      console.error('Usage: TURSO_DATABASE_URL=libsql://your-db.turso.io TURSO_AUTH_TOKEN=your-token node scripts/seed-blogs.mjs');
      process.exit(1);
    }
